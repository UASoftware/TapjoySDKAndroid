...
public class TapjoyConnectCore
{
	...
	private static String storeName = "";												// Android app/market store to use.
...
//	private static boolean enableVideoCache = true;										// Whether video caching is enabled or not.
	...
	private static Hashtable<String, String> connectFlags = null;						// Flags set in requestTapjoyConnect hashtable or manifest meta-data.
	...
	private static String getParamsWithoutAppID()
	{
		...
		{
			urlParams += "&";
			urlParams += TapjoyConstants.TJC_STORE_NAME + "=" + Uri.encode(storeName);
		}
	...
	private void init()
	{
		...
		// Find any connect flags in the AndroidManifest meta data.
		try
		{
			...
			if (manager != null)
			{
				...
				// Check if metadata exists within <application> tag.
				if (appInfo != null && appInfo.metaData != null)
				{
					for (String key : TapjoyConnectFlag.FLAG_ARRAY)
					{
						...
						if (obj != null)
						{
							...
							if (value != null)
							{
								TapjoyLog.i(TAPJOY_CONNECT, "Found manifest flag: " + key + ", " + value);
				...
				{
					TapjoyLog.i(TAPJOY_CONNECT, "No metadata present.");
				}
			}
		}
		catch (Exception e)
		{
			TapjoyLog.e(TAPJOY_CONNECT, "Error reading manifest meta-data: " + e.toString());
		}
		...
		try 
		{
			...
			// Alternate store.
			if (getFlagValue(TapjoyConnectFlag.STORE_NAME) != null && getFlagValue(TapjoyConnectFlag.STORE_NAME).length() > 0)
			{
				storeName = getFlagValue(TapjoyConnectFlag.STORE_NAME);
				
				// Check for existing/supported store names.
				ArrayList <String> supportedStores = new ArrayList<String>(Arrays.asList(TapjoyConnectFlag.STORE_ARRAY));
				
				// Warning for undefined store names.
				if (!supportedStores.contains(storeName))
				{
					Log.w(TAPJOY_CONNECT, "Warning -- undefined STORE_NAME: " + storeName);
				}
			}
			...
			TapjoyLog.i(TAPJOY_CONNECT, "" + TapjoyConstants.TJC_STORE_NAME + ": [" + storeName + "]");
			...
			if (connectFlags != null)
			{
				...
				while (iterator.hasNext())
				{
					...
					// sha_2_udid is only supported in the Advertiser SDK, so remove it if it's present in a non-Advertiser SDK.
					if (item.getKey().equals(TapjoyConnectFlag.SHA_2_UDID) && !sdkType.equals(TapjoyConstants.TJC_SDK_TYPE_CONNECT))
					{
						TapjoyLog.w(TAPJOY_CONNECT, "WARNING -- only the Connect/Advertiser SDK can support " + TapjoyConnectFlag.SHA_2_UDID);
...
//	}
	...
	/**
	 * Returns the video parameters to send when requesting ads.
	 * @return								Video related parameters to send when requesting an ad unit.
	 */
	public static String getVideoParams()
	{
		...
		if (videoEnabled)
		{
			// Send any video ids we've cached.  Server will determine whether to just show
			// cached videos or all videos.
			if (videoIDs.length() > 0)
				params = TapjoyConstants.TJC_VIDEO_OFFER_IDS + "=" + videoIDs;
		}
		else
		{
			// Hide videos if videos are disabled.
			params = TapjoyConstants.TJC_HIDE_VIDEOS + "=" + "true";
		}
		
		TapjoyLog.i(TAPJOY_CONNECT, "video parameters: " + params);
